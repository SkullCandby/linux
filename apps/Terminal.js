...
const com_import=async(term, args)=>{//«
const terr=(arg)=>{term_error(term, arg);return TERM_ERR;};
for (let arg of args){
	if (NS.coms[arg]) {
	...
	try{
		...
		terr(`${arg}: Error importing the module (see the javascript console)`);
log(e);
	}
}
};//»

//System commands (leading underscore) down here
