...
console.warn("WALTER'S COMPILER!!!!");
...
function semantics$2({ stmt }) {
  ...
  function arrayOffset(base, offset) {
    const shift = shifts$1[base.meta.TYPE_ARRAY] || 2;

    return offset.Type !== Syntax.Constant || Number(offset.value) ? stmt`(${base} + (${offset} << ${shift}));` : stmt`(${base});`;
  }
  ...
  return {
    ...
    [Syntax.ArraySubscript]: next => (args, t) => {
      ...
      return extendNode({ meta: { TYPE_ARRAY } }, transform(stmt`${type}.load(${index});`));
    },
...
const makeStruct = stmt => (base, field) => {
    ...
    semantics({ stmt }) {
      ...
      return {
            ...
            [Syntax.UnionType]: () => {
              walkNode({
                ...
                [Syntax.Identifier]: id => {
                  const structReference = userTypes[transform([id, context]).value];
        ...
        [Syntax.ArraySubscript]: next => (args, t) => {
          ...
          }, stmt`(${base} + (${offset} * sizeof(${parsed.meta.TYPE_ARRAY})));`), context]);
        }
      };
    }
  };
...
function generateErrorString(msg, error, marker, filename, func) {
  ...
  const highlight = new Array(end - col + 1).join('^').padStart(marker.start.col - 1, ' ');
...
//DTHGEUITMH
/*
Does the '2' (CONSTANT_2) refer to the FINAL_END_BYTE shown below
as def.End.code PLUS the imagined lone byte that it takes to report
the number of locals in the locals stream (but only if the number of
locals can fit into 7 bits, e.g. < 0x80)
*/
  let funcSz = body.size + localsStream.size + 2;//<- CONSTANT_2
//If so, then adjust the reported size accordingly...
  funcSz += (encodeUnsigned(locals.length).length - 1);
  stream.push(varuint32, funcSz, functionName);
